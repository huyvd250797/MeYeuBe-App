...
for f in ['index.html','app.js','manifest.webmanifest','sw.js','version.md']:
    if '14.3.0' not in (root/f).read_text(encoding='utf-8'): errors.append(f+' chưa đồng bộ version')

for required_file in ['AC_V14.3.0.md','BASELINE_LOCK_V14.3.0.json','PUSH_NOTIFICATION_SETUP.md','supabase/functions/send-push/index.ts']:
...
PREV_LOCK='BASELINE_LOCK_V14.2.0.json'
...
# V14.3.0: rong. Animation System khong sua bat ky ham cu nao — moi can thiep deu
# di qua axWrap() luc chay, nen 75 ham trong BASELINE_LOCK_V14.2.0.json giu nguyen hash.
...
if errors:
    ...
    sys.exit(1)
print('RELEASE CHECK PASSED: V14.3.0')
