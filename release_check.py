...
for f in ['index.html','app.js','manifest.webmanifest','sw.js','version.md']:
    if '10.8.4' not in (root/f).read_text(encoding='utf-8'): errors.append(f+' chưa đồng bộ version')

for required_file in ['AC_V10.8.4.md','PUSH_NOTIFICATION_SETUP.md','supabase/functions/send-push/index.ts']:
...
if errors:
    ...
    sys.exit(1)
print('RELEASE CHECK PASSED: V10.8.4')
