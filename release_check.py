#!/usr/bin/env python3
"""Release smoke check for Mẹ Yêu Bé V15.0.7."""
...
for name, txt in {
    ...
    if "15.0.7" not in txt and "V15.0.7" not in txt:
        errors.append(f"{name} chưa đồng bộ V15.0.7")

# Cache busting / boot guard
for token in ['src="./boot.js?v=15.0.7"', 'src="./app.js?v=15.0.7"', 'ME YEU BE · V15.0.7', '<b>V15.0.7</b>']:
    if token not in idx:
        errors.append("index.html thiếu token version/cache: " + token)
for token in ["var APP_VERSION=\"15.0.7\"", "V15.0.7 · TimerChipFix"]:
    if token not in app:
        errors.append("app.js thiếu token V15.0.7: " + token)
for token in ["var BUILD='15.0.7'", "build.json", "MEYEUBE_BUILD_ACK"]:
    if token not in boot:
        errors.append("boot.js thiếu boot guard/version: " + token)
for token in ["const BUILD='15.0.7'", "cache:'no-store'", "caches.delete(k)"]:
    if token not in sw:
        errors.append("sw.js thiếu SW guard/version: " + token)

# V15.0.7 scroll-lock acceptance checks
for token in [
    ...
    if token not in (idx + app):
        errors.append("Thiếu cơ chế khóa scroll V15.0.7: " + token)


# V15.0.7 hotfix: Pull-to-refresh không được hoạt động khi sheet đang mở
for token in [
    ...
    if token not in (idx + app):
        errors.append("Thiếu hotfix V15.0.7: " + token)


# V15.0.7 UXFix acceptance checks
for token in [
    "mybOverlayCore",
    "feedTimerStart",
    "bcStatusFeeding",
    ...
    if token not in (idx + app):
        errors.append("Thiếu UXFix V15.0.7: " + token)
...
for required in ["AC_V15.0.7.md", "PUSH_NOTIFICATION_SETUP.md", "supabase/functions/send-push/index.ts"]:
...
if errors:
    ...
    for e in errors:
        print("- " + e)
    sys.exit(1)
print("RELEASE CHECK PASSED: V15.0.7")
