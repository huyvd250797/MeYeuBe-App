...
for f in ['index.html','app.js','manifest.webmanifest','sw.js','version.md']:
    if '10.9.0' not in (root/f).read_text(encoding='utf-8'): errors.append(f+' chưa đồng bộ version')

for required_file in ['AC_V10.9.0.md','PUSH_NOTIFICATION_SETUP.md','supabase/functions/send-push/index.ts']:
