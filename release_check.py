...
for token in ['careMetrics','openCareEventFromDashboard','nextFeedHours','medicine','temperature','spitup','cloudRealtimeStart','cloudMergePayloads','evaluateSmartAlerts','openSmartAlertCenter','normalizeSmartAlertConfig']:
...
for f in ['index.html','app.js','manifest.webmanifest','sw.js','version.md']:
    if '10.7.1' not in (root/f).read_text(encoding='utf-8'): errors.append(f+' chưa đồng bộ version')
if errors:
    print('RELEASE CHECK FAILED')
    [print('- '+e) for e in errors]
    sys.exit(1)
print('RELEASE CHECK PASSED: V10.7.1')
