#!/usr/bin/env python3
"""Release smoke check for Mẹ Yêu Bé V15.0.6."""
...
for name, txt in {
    ...
    if "15.0.6" not in txt and "V15.0.6" not in txt:
        errors.append(f"{name} chưa đồng bộ V15.0.6")

# Cache busting / boot guard
for token in ['src="./boot.js?v=15.0.6"', 'src="./app.js?v=15.0.6"', 'ME YEU BE · V15.0.6', '<b>V15.0.6</b>']:
    if token not in idx:
        errors.append("index.html thiếu token version/cache: " + token)
for token in ["var APP_VERSION=\"15.0.6\"", "V15.0.6 · FeedStatus TimelineUX — Timer bú + trạng thái + Timeline chip"]:
    if token not in app:
        errors.append("app.js thiếu token V15.0.6: " + token)
for token in ["var BUILD='15.0.6'", "build.json", "MEYEUBE_BUILD_ACK"]:
    if token not in boot:
        errors.append("boot.js thiếu boot guard/version: " + token)
for token in ["const BUILD='15.0.6'", "cache:'no-store'", "caches.delete(k)"]:
    if token not in sw:
        errors.append("sw.js thiếu SW guard/version: " + token)

# V15.0.6 scroll-lock acceptance checks
for token in [
    ...
    if token not in (idx + app):
        errors.append("Thiếu cơ chế khóa scroll V15.0.6: " + token)


# V15.0.6 hotfix: Pull-to-refresh không được hoạt động khi sheet đang mở
for token in [
    ...
    if token not in (idx + app):
        errors.append("Thiếu hotfix V15.0.6: " + token)


# V15.0.6 UXFix acceptance checks
for token in [
    "mybOverlayCore",
    "feedTimerStart",
    "bcStatusFeeding",
    ...
    "tl8ActionOnlyBtn",
    "tl8ActDanger",
    "tl8DetailActionScroller",
    "tl8ResetView",
    "AX_PRESS_SEL='button,a",
]:
    if token not in (idx + app):
        errors.append("Thiếu UXFix V15.0.6: " + token)
...
for required in ["AC_V15.0.6.md", "PUSH_NOTIFICATION_SETUP.md", "supabase/functions/send-push/index.ts"]:
...
if errors:
    ...
    for e in errors:
        print("- " + e)
    sys.exit(1)
print("RELEASE CHECK PASSED: V15.0.6")
