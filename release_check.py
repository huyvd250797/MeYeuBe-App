#!/usr/bin/env python3
"""Release smoke check for Mẹ Yêu Bé V15.0.47."""
...
for name, txt in {
    ...
    if "15.0.47" not in txt and "V15.0.47" not in txt:
        errors.append(f"{name} chưa đồng bộ V15.0.47")

# Cache busting / boot guard
for token in ['src="./boot.js?v=15.0.47"', 'src="./app.js?v=15.0.47"', 'ME YEU BE · V15.0.47', '<b>V15.0.47</b>']:
    if token not in idx:
        errors.append("index.html thiếu token version/cache: " + token)
for token in ["var APP_VERSION=\"15.0.47\"", "V15.0.47 · PumpMilk24UI"]:
    if token not in app:
        errors.append("app.js thiếu token V15.0.47: " + token)
for token in ["var BUILD='15.0.47'", "build.json", "MEYEUBE_BUILD_ACK"]:
    if token not in boot:
        errors.append("boot.js thiếu boot guard/version: " + token)
for token in ["const BUILD='15.0.47'", "cache:'no-store'", "caches.delete(k)"]:
    if token not in sw:
        errors.append("sw.js thiếu SW guard/version: " + token)


# V15.0.47 StartupCloudPreloadFix acceptance
for token in [
    ...
    if token not in (idx + app):
        errors.append("Thiếu StartupCloudPreloadFix V15.0.47: " + token)

# V15.0.47 scroll-lock acceptance checks
for token in [
    ...
    if token not in (idx + app):
        errors.append("Thiếu cơ chế khóa scroll V15.0.47: " + token)


# V15.0.47 hotfix: Pull-to-refresh không được hoạt động khi sheet đang mở
for token in [
    ...
    if token not in (idx + app):
        errors.append("Thiếu hotfix V15.0.47: " + token)


# V15.0.47 UXFix acceptance checks
for token in [
    "mybOverlayCore",
    "feedTimerStart",
    "bcStatusFeeding",
    ...
    if token not in (idx + app):
        errors.append("Thiếu UXFix V15.0.47: " + token)


# V15.0.47 MilkFeedFix acceptance checks
for token in ["v1511-milk-feed-fix", "v1512-milk-scroll-swipe-fix", "milkChosenExpire", "window.abOnAmountInput=function", ".milkSwipeShell,.milkSwipeActions", "PumpMilk24UI"]:
    if token not in (idx + app):
        errors.append("Thiếu MilkFeedFix V15.0.47: " + token)


# V15.0.47 PumpMilk24UI acceptance checks
for token in ["careRecordSwipeStart=function", "mcIsBusyForPump", "v1514-pump-swipe-fix", "Bình/túi này đang Tạm ẩn"]:
    if token not in (idx + app):
        errors.append("Thiếu PumpMilk24UI V15.0.47: " + token)
...
for required in ["AC_V15.0.47.md", "PUSH_NOTIFICATION_SETUP.md", "supabase/functions/send-push/index.ts", "supabase/functions/smart-alert-cron/index.ts", "docs/SMART_ALERT_CRON_SETUP.md"]:
...
# V15.0.47 SmartAlertCronPush acceptance
for token in ["SmartAlertCronPush", "normalizePumpExclusiveLinks", "duplicate_pump_link", "linked_to_foreign_pump_bag", "Bình \"" ]:
    if token not in (idx + app):
        errors.append("Thiếu SmartAlertCronPush V15.0.47: " + token)
if not (root / "AC_V15.0.47.md").exists():
    errors.append("Thiếu file: AC_V15.0.47.md")


# V15.0.47 Smart Alert Cron Push acceptance
for token in ["smart-alert-cron", "VAPID_PRIVATE_KEY", "push_delivery_log", "Nhắc sau 15 phút", "không thiết bị nào đang mở app"]:
    if token not in (idx + app + read("PUSH_NOTIFICATION_SETUP.md") + read("docs/SMART_ALERT_CRON_SETUP.md") + read("supabase/functions/smart-alert-cron/index.ts")):
        errors.append("Thiếu Smart Alert Cron Push V15.0.47: " + token)


# V15.0.47 StoredFeedFastAutoFix acceptance
for token in ["StoredFeedFastAutoFix", "adjustedSourcesForNeed", "Chỉ bấm ✕ mới chuyển sang thủ công", "lượng sữa của túi sẽ được trả lại kho"]:
    if token not in (idx + app + changelog + version):
        errors.append("Thiếu StoredFeedFastAutoFix V15.0.47: " + token)
if not (root / "AC_V15.0.47.md").exists():
    errors.append("Thiếu file: AC_V15.0.47.md")
...
if errors:
    ...
    for e in errors:
        print("- " + e)
    sys.exit(1)
print("RELEASE CHECK PASSED: V15.0.47")


# V15.0.47 InventorySafeFix acceptance
for token in ["v1521-search-nav-loading-fix", "gsStrictTokenHitV1521", "body.menuOpen .bottomNav", "loadingLogo img", "rawType==='feed'||rawType==='pump'||rawType==='spitup'"]:
    if token not in (idx + app):
        errors.append("Thiếu InventorySafeFix V15.0.47: " + token)
