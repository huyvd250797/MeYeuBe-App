...
function milkBagCreatedLabel(b){var raw=String((b&&(b.createdAt||b.createdDateTime||b.created))||'');var d=(b&&(b.date||b.startDate))||'';var t=(b&&(b.timeFrom||b.time))||'';if(raw){var datePart=raw.slice(0,10);var m=raw.match(/(?:T|\s)(\d{2}:\d{2})/);return fmtDate(datePart)+((m&&m[1])?' '+m[1]:'')}return d?(fmtDate(d)+(t?' '+t:'')):'--'}
function milkBagCreatedSortValue(b){var raw=String((b&&(b.createdAt||b.createdDateTime||b.created))||'');if(raw)return raw;var d=(b&&(b.date||b.startDate))||'';var t=(b&&(b.timeFrom||b.time))||'00:00';return d?(d+'T'+t):''}
function milkBagOptionText(b){return milkUrgencyIcon(b)+' '+b.id+' · tạo '+milkBagCreatedLabel(b)+' · còn '+(b.remaining||0)+'ml · '+(b.storage||'')+' · '+milkTimeLeftText(b)}
...
function careEventDetailSortDesc(a,b){var ka=((a.startDate||a.date||'')+'T'+(a.timeFrom||'00:00'));var kb=((b.startDate||b.date||'')+'T'+(b.timeFrom||'00:00'));return kb.localeCompare(ka)}
...
function renderCareStatDetail(type,date){
  ...
  var arr=(type==='milk')?(db.milkInventory||[]).slice().sort(function(a,b){return milkBagCreatedSortValue(b).localeCompare(milkBagCreatedSortValue(a)) || milkExpireAt(a)-milkExpireAt(b)}):careEventsForDate(db,date,type).sort(careEventDetailSortDesc);var title=careTypeDetailTitle(type);
  var body='';
