...
function openCareStatsFromDashboard(type){
  var selected=(type&&type!=='schedule')?type:'';
  window.__careStatsSelectedType=selected;
  var d=today();
  ...
  if(selected){
    setTimeout(function(){renderCareStatDetail(selected,d)},100);
  }else{
    setTimeout(function(){var target=byId('careStatsBox');if(target&&target.scrollIntoView)target.scrollIntoView({behavior:'smooth',block:'start'})},80);
  }
}
...
function getDashboardConfig(db){
  ...
  return {
    ...
    nextFeedIntervalHours:(function(v){v=Number(v);return isFinite(v)&&v>0?v:2.5})(cfg.nextFeedIntervalHours)
  };
}
...
function renderDashboardConfig(){
  ...
  if(byId('cfgNextFeedIntervalHours'))byId('cfgNextFeedIntervalHours').value=cfg.nextFeedIntervalHours||2.5;
...
function readDashboardConfigFromForm(){
  ...
  var nextFeedHours=Number(byId('cfgNextFeedIntervalHours')&&byId('cfgNextFeedIntervalHours').value);
  cfg.nextFeedIntervalHours=isFinite(nextFeedHours)&&nextFeedHours>0?nextFeedHours:2.5;
