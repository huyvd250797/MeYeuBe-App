...
function milkBagDateCodeFromDate(date){var d=String(date||today()).slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(d))d=today();return d.slice(2,4)+d.slice(5,7)+d.slice(8,10)}
function shortMilkBagCode(b){var raw=String((b&&b.id)||'');if(/^\d{6}(?:-\d{2})?$/.test(raw))return raw;return milkBagDateCodeFromDate((b&&(b.date||b.startDate))||today())}
function newMilkBagIdForItem(item,db){var base=milkBagDateCodeFromDate((item&&item.date)||today());var arr=(db&&db.milkInventory)||[];if(!arr.some(function(b){return b.id===base}))return base;for(var i=2;i<100;i++){var id=base+'-'+String(i).padStart(2,'0');if(!arr.some(function(b){return b.id===id}))return id}return base+'-'+Date.now().toString().slice(-4)}
function milkBagCreatedText(b){var raw=String((b&&(b.createdAt||b.createdDateTime||b.created))||'');var d=(b&&(b.date||b.startDate))||'';var t=(b&&(b.timeFrom||b.time))||'';if(raw){var m=raw.match(/^(\d{4}-\d{2}-\d{2})(?:T|\s)(\d{2}:\d{2})/);if(m){d=m[1];t=m[2]}}return (d?fmtDate(d):'--')+(t?' '+t:'')}
function milkBagNoteText(b){var n=String((b&&b.note)||'').trim();return n?(' · Ghi chú: '+n):''}
function milkBagOptionText(b){return milkUrgencyIcon(b)+' '+shortMilkBagCode(b)+' · tạo '+milkBagCreatedText(b)+' · còn '+(b.remaining||0)+'ml · '+(b.storage||'')+milkBagNoteText(b)+' · '+milkTimeLeftText(b)}
...
function renderCareDynamicFields(type,db){
  ...
  if(type==='feed'){
    ...
    box.innerHTML='<div class="crossDayHint">Nhập Ngày bắt đầu/Từ giờ. Đến giờ không bắt buộc: nếu để trống, app hiểu là bé đang ngủ và dự kiến dậy sau 3 giờ. Nếu nhập Đến giờ, app hiểu là bé đã dậy vào thời gian đó.</div>';
  }else if(type==='diaper'){
...
function getCareEventFromForm(db){
  ...
  if(type==='sleep'){item.unit='phút';item.extra=item.extra||{};if(!timeTo){item.timeTo='';item.endDate=startDate;item.amount=0;item.extra.isOngoing=true;item.extra.expectedWakeAt=localDateTimeValue(new Date(dateTimeMs(startDate,timeFrom)+3*3600000));}else{item.extra.isOngoing=false;item.amount=minutesBetweenDateTimes(startDate,timeFrom,endDate,timeTo)}}
...
function careEventSortValue(x){return String((x&&x.startDate)||((x&&x.date)||''))+'T'+String((x&&x.timeFrom)||'00:00')}
function careSortDesc(a,b){return careEventSortValue(b).localeCompare(careEventSortValue(a))}
function sleepExpectedWakeText(x){var raw=x&&x.extra&&x.extra.expectedWakeAt;if(!raw&&x){var ms=dateTimeMs(x.startDate||x.date,x.timeFrom);if(ms!==null)raw=localDateTimeValue(new Date(ms+3*3600000))}if(!raw)return '';var d=String(raw).slice(0,10),t=String(raw).slice(11,16);return t+', '+fmtDate(d)}
function currentOpenSleepEvent(db){var arr=((db&&db.careEvents)||[]).filter(function(x){return x&&x.type==='sleep'&&(!x.timeTo||(x.extra&&x.extra.isOngoing))});return arr.sort(careSortDesc)[0]||null}
function sleepStatusInfo(db){var x=currentOpenSleepEvent(db);if(x)return {sleeping:true,event:x,wakeText:sleepExpectedWakeText(x)};return {sleeping:false,event:null,wakeText:''}}
...
function careEventsForDate(db,date,type){var mapped=(db.careEvents||[]).map(function(x,i){var y=Object.assign({},x);y._idx=i;return y});if(type==='pee'||type==='poop'){return mapped.filter(function(x){if((x.startDate||x.date)!==date)return false;if(x.type===type)return true;if(x.type==='diaper')return type==='pee'?diaperPeeCount(x)>0:diaperPoopCount(x)>0;return false}).map(function(x){if(x.type==='diaper'){var y=Object.assign({},x);y._derivedType=type;y._derivedAmount=type==='pee'?diaperPeeCount(x):diaperPoopCount(x);return y}return x}).sort(careSortDesc)}return mapped.filter(function(x){if(type&&x.type!==type)return false;if(x.type==='sleep'){if(!x.timeTo||(x.extra&&x.extra.isOngoing))return (x.startDate||x.date)===date;return careOverlapMinutesOnDate(x,date)>0}return (x.startDate||x.date)===date}).sort(careSortDesc)}
function careTypeDetailTitle(type){var m=careTypeMeta(type);return m.icon+' '+m.label}
function milkBagLabel(db,id,snapshot){if(!id)return '';if(snapshot){return shortMilkBagCode(snapshot)+' · '+(snapshot.amount||0)+'ml · dùng '+(snapshot.used||0)+'ml · còn sau bú '+(snapshot.remainingAfter||0)+'ml · '+(snapshot.statusAfter||'')+(snapshot.expireDateTime?' · HSD '+fmtMilkExpire({expireDateTime:snapshot.expireDateTime})+' · '+milkTimeLeftText({expireDateTime:snapshot.expireDateTime}):'')}var b=findMilkBag(db,id);return b?shortMilkBagCode(b)+' · '+(b.amount||0)+'ml · còn hiện tại '+(b.remaining||0)+'ml · '+(b.status||'')+(b.expireDate||b.expireDateTime?' · HSD '+fmtMilkExpire(b)+' · '+milkTimeLeftText(b):''):'Túi '+id}
...
function careDetailHtml(db,x){var displayType=x._derivedType||x.type;var meta=careTypeMeta(displayType);var rows=[];rows.push('Thời gian: '+eventDateRangeLabel(x));
  ...
  if(x.type==='sleep'){if((!x.timeTo)||(x.extra&&x.extra.isOngoing)){rows.push('Trạng thái: Bé đang ngủ');rows.push('Thời gian dậy dự kiến: '+sleepExpectedWakeText(x));}else{rows.push('Tổng ngủ: '+fmtMinutes(x.amount||0));}}
...
function renderCareStatDetail(type,date){
  ...
  var arr=(type==='milk')?(db.milkInventory||[]).slice().sort(function(a,b){return careEventSortValue({startDate:b.date||String(b.createdAt||'').slice(0,10),timeFrom:b.timeFrom||String(b.createdAt||'').slice(11,16)}).localeCompare(careEventSortValue({startDate:a.date||String(a.createdAt||'').slice(0,10),timeFrom:a.timeFrom||String(a.createdAt||'').slice(11,16)}))}):careEventsForDate(db,date,type);var title=careTypeDetailTitle(type);
  var body='';
  if(!arr.length){body='<p class="notice">Không có dữ liệu chi tiết cho loại này trong ngày đã chọn.</p>'}
  else if(type==='milk'){body=arr.map(function(b){var isActive=(b.status||'Đang bảo quản')==='Đang bảo quản';var rawCreated=String(b.createdAt||b.createdDateTime||b.created||'');var createdDate=rawCreated?fmtDate(rawCreated.slice(0,10)):(b.date?fmtDate(b.date):'--');var mTime=rawCreated.match(/(?:T|\s)(\d{2}:\d{2})/);var createdTime=(mTime&&mTime[1])||(b.timeFrom||b.time||'');var created=createdDate+(createdTime?' - '+createdTime:'');var cls=isActive?'':' disabled';return '<div class="careDetailItem milkBagDetail'+cls+'"><b>'+esc(milkUrgencyIcon(b)+' '+shortMilkBagCode(b)+' · '+(b.remaining||0)+'/'+(b.amount||0)+'ml')+(isActive?'':' <span class="disabledTag">Không khả dụng</span>')+'</b><small>'+esc(created+' · Trạng thái: '+(b.status||'')+' · Bảo quản: '+(b.storage||'--')+(b.expireDate||b.expireDateTime?' · HSD '+fmtMilkExpire(b)+' · '+milkTimeLeftText(b):''))+'</small>'+(b.note?'<p>'+esc(b.note)+'</p>':'')+(b.cancelReason?'<p><b>Lý do huỷ:</b> '+esc(b.cancelReason)+'</p>':'')+'</div>'}).join('')}
...
function careChartUnit(type){if(type==='feed'||type==='pump')return 'ml';if(type==='sleep')return 'giờ';if(type==='diaper')return 'tã';return 'lần'}
...
function cancelMilkBag(id){var db=load();var b=(db.milkInventory||[]).find(function(x){return x.id===id});if(!b){showToast('Không tìm thấy túi sữa','error');return}if((b.status||'Đang bảo quản')!=='Đang bảo quản'){showToast('Túi sữa này đã không còn khả dụng','warn');return}var reason=prompt('Nhập lý do huỷ túi sữa '+shortMilkBagCode(b)+':','');if(reason===null)return;reason=String(reason||'').trim();if(!reason){showToast('Vui lòng nhập lý do huỷ túi','warn');return}if(!confirm('Xác nhận huỷ túi sữa '+shortMilkBagCode(b)+'? Túi này sẽ chuyển sang trạng thái Đã bỏ và không dùng lại được.'))return;b.status='Đã bỏ';b.cancelReason=reason;b.canceledAt=new Date().toISOString();b.remaining=0;b.updatedAt=new Date().toISOString();save(db);showToast('Đã huỷ túi sữa '+shortMilkBagCode(b),'success')}
function initMilkBagSwipe(){document.querySelectorAll('.milkBagSwipe').forEach(function(row){if(row.__swipeReady)return;row.__swipeReady=true;var sx=0,sy=0;row.addEventListener('touchstart',function(e){var t=e.touches&&e.touches[0];if(!t)return;sx=t.clientX;sy=t.clientY},{passive:true});row.addEventListener('touchend',function(e){var t=e.changedTouches&&e.changedTouches[0];if(!t)return;var dx=t.clientX-sx,dy=t.clientY-sy;if(Math.abs(dx)>50&&Math.abs(dx)>Math.abs(dy)){document.querySelectorAll('.milkBagSwipe.showActions').forEach(function(x){if(x!==row)x.classList.remove('showActions')});row.classList.toggle('showActions',dx<0);}}, {passive:true});})}
function renderMilkInventory(db){var box=byId('milkInventoryBox');if(!box)return;var arr=(db.milkInventory||[]).slice().sort(function(a,b){var ar=(a.status==='Đang bảo quản'?0:a.status==='Đã sử dụng hết'?1:2),br=(b.status==='Đang bảo quản'?0:b.status==='Đã sử dụng hết'?1:2);return ar-br || milkExpireAt(a)-milkExpireAt(b)});if(!arr.length){box.innerHTML='<p class="notice">Chưa có kho sữa. Khi ghi nhận Hút sữa, app sẽ tự tạo túi sữa ở đây.</p>';return}box.innerHTML=arr.map(function(b){var status=b.status||'Đang bảo quản';var active=status==='Đang bảo quản';var cls=(status==='Đã sử dụng hết'?'used finished ':'')+(milkExpireAt(b)<Date.now()?'expired ':'')+(active?'':' disabled');var code=shortMilkBagCode(b);var body='<div class="milkBag '+cls+'"><b>'+esc(milkUrgencyIcon(b)+' '+code+' · '+(b.remaining||0)+'/'+(b.amount||0)+'ml · '+status)+(active?'':' <span class="disabledTag">Không khả dụng</span>')+'</b><small>Ngày tạo/lưu trữ: '+esc(milkBagCreatedText(b))+' · Hút: '+fmtDate(b.date)+' '+esc(timeRangeOf(b)||b.timeFrom||'')+' · '+esc(b.storage||'')+(b.expireDate||b.expireDateTime?' · HSD '+esc(fmtMilkExpire(b))+' · '+esc(milkTimeLeftText(b)):'')+'</small>'+(b.note?'<p>'+esc(b.note)+'</p>':'')+(b.cancelReason?'<p><b>Lý do huỷ:</b> '+esc(b.cancelReason)+'</p>':'')+'</div>';if(active){return '<div class="milkBagSwipe" data-bag-id="'+esc(b.id)+'"><div class="milkBagActions"><button type="button" onclick="cancelMilkBag(\''+esc(b.id)+'\')">Huỷ túi</button></div>'+body+'</div>'}return '<div class="milkBagSwipe">'+body+'</div>'}).join('');setTimeout(initMilkBagSwipe,0)}
...
function renderCloudConfig(){
  ...
  if(p){p.textContent=cfg.enabled?'ON':'OFF';p.classList.toggle('off',!cfg.enabled)} updateCloudNetworkStatus();
}
...
async function cloudFetchRow(cfg){
  ...
  var url=cloudEndpoint(cfg)+'?id=eq.'+encodeURIComponent(cfg.syncId)+'&select=id,data,updated_at';
  var res=await fetch(url,{headers:cloudHeaders(cfg)});
  if(!res.ok){throw new Error('Cloud fetch lỗi '+res.status+': '+await res.text())}
  var rows=await res.json();
  if(rows&&rows[0]){
    return {sync_id:rows[0].id,payload:rows[0].data,updated_at:rows[0].updated_at};
  }
  return null;
}
async function cloudUpsertPayload(cfg,payload){
  cloudValidateCfg(cfg);
  var body={id:cfg.syncId,data:payload,updated_at:new Date().toISOString()};
...
function updateCloudNetworkStatus(){
  var el=byId('cloudNetworkStatus');
  if(!el)return;
...
window.addEventListener('online',updateCloudNetworkStatus);window.addEventListener('offline',updateCloudNetworkStatus);
