...
function syncCareFormChromeForType(type){
  /* V15.0.5: Timer chỉ còn đúng một nút "Bắt đầu bú" và chỉ hiện trong form Bé bú.
...
function renderCareTimerState(){var box=byId('careTimerBox');if(!box)return;var selected=window.__careSelectedType||'feed';var t=loadCareTimer();var feedRunning=t&&t.type==='feed';box.classList.toggle('hidden',selected!=='feed'&&!feedRunning);if(selected!=='feed'&&!feedRunning){box.innerHTML='';return}if(!feedRunning){box.innerHTML='<div class="careTimerActions careTimerActionsCompact"><button type="button" class="feedTimerStart" onclick="startCareTimer()" aria-label="Bắt đầu đếm thời gian bé bú"><span>⏱</span><b>Bắt đầu bú</b></button></div>';return}var sec=Math.max(0,Math.floor((Date.now()-new Date(t.startedAt).getTime())/1000)),h=Math.floor(sec/3600),m=Math.floor((sec%3600)/60),ss=sec%60;box.innerHTML='<div class="careTimerRunning careTimerFeedRunning"><b>🍼 Đang bú</b><span>'+String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':'+String(ss).padStart(2,'0')+'</span><div><button type="button" class="ok" onclick="stopCareTimer()">Dừng & điền giờ</button><button type="button" class="danger" onclick="cancelCareTimer()">Hủy</button></div></div>'}
...
function babyStatusMeta(db){
  ...
  if(feedSecs!==null)return {text:'🍼 Bé đang bú',hint:'Đã bú được '+fmtDurationVN(feedSecs),cls:'bcStatusFeeding',click:false};
...
function syncSleepElapsedUI(){
  ...
  var feedSecs=babyFeedElapsedSeconds();
  if(feedSecs!==null){el.textContent='Đã bú được '+fmtDurationVN(feedSecs);return}
...
function renderDashboard(db){
  ...
  blocks.babyInfo=function(){
    ...
    var statusMeta=babyStatusMeta(db),isSleeping=statusMeta.click,nextFeed=nextFeedText(db);h+='<div class="bcStatusBar"><div class="bcStatus '+esc(statusMeta.cls)+(isSleeping?' bcStatusClickable':'')+'" '+(isSleeping?'role="button" tabindex="0" onclick="editLatestActiveSleepFromDashboard()" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){editLatestActiveSleepFromDashboard()}"':'')+'>'+esc(statusMeta.text)+(statusMeta.hint?'<span class="bcSleepHint" id="bcSleepElapsed">'+esc(statusMeta.hint)+'</span>':'')+'</div><div class="bcClock"><span>🕘 <span id="vnClock">--:--:--</span></span><span class="bcTodayDate">'+esc(weekdayDateLine(todayStr))+'</span></div></div>';h+='<div class="bcStatusExtra" id="bcNextFeedWrap">'+nextFeedLineHtml(db)+'</div>';
    h+='</section>';return h;
  };
...
function tl8Detail(id){
  ...
  h+='<div class="tl8DetailIconBar" aria-label="Thao tác bản ghi">'+
       '<button type="button" class="primary" onclick="tl8Edit(\''+esc(id)+'\')" title="Sửa"><span>✏️</span><b>Sửa</b></button>'+
       '<button type="button" onclick="tl8Duplicate(\''+esc(id)+'\')" title="Nhân bản"><span>📄</span><b>Nhân</b></button>'+
       '<button type="button" class="toggle'+favCls+'" onclick="tl8ToggleFav(\''+esc(id)+'\')" title="Yêu thích"><span>⭐</span><b>Yêu thích</b></button>'+
       '<button type="button" class="toggle'+pinCls+'" onclick="tl8TogglePin(\''+esc(id)+'\')" title="Ghim"><span>📌</span><b>Ghim</b></button>'+
       '<button type="button" onclick="tl8OpenSheet(\''+esc(id)+'\',\'full\')" title="Thêm thao tác"><span>⋯</span><b>Thêm</b></button>'+
     '</div>';
  h+='</div>';
  body.innerHTML=h;
  tl8Show(ov);
...
/* ============================================================================
   V15.0.5 · UXFix — SheetCore + Timeline gọn + Timer bú
   ============================================================================ */
(function(){
  var OPEN_SEL=[
    '.tl8Sheet.show','.tl8Overlay.show','.moreSheet.show','.streakOverlay.show',
  ...
  if(typeof AX_PRESS_SEL!=='undefined')AX_PRESS_SEL='.bcMetric,.bcTimeRow,.bcApptCard,.dashCareCell,.navItem,.careEvent,.moreItem,.scheduleDue,.careStatBox,.diaperChoice,[role=button],[onclick]';
  if(typeof AX_TAP_SEL!=='undefined')AX_TAP_SEL='button,a,[role=button],[onclick],.bcMetric,.dashCareCell,.navItem,.careEvent,.moreItem,.careStatBox,.bcApptCard,.diaperChoice';
