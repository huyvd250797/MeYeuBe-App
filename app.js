...
function gsBuildIndex(){
  ...
  (db.careEvents||[]).forEach(function(x,i){
    ...
    var blob=gsDeaccent(blobRaw+' '+gsDateForms(iso).join(' '));
...
function gsRender(){
  ...
  if(!list.length){
    box.innerHTML='<div class="gsEmpty"><span>🔍</span><b>'+(hasFilter?'Không tìm thấy kết quả':'Nhập từ khóa để tìm kiếm')+'</b><small>'+
      (hasFilter?'Thử từ khóa khác: mã túi sữa, số ml, ngày (24/07), loại (bú, ngủ, thuốc), tên thuốc, milestone…':'Tìm theo ngày, giờ, số ml, mã túi sữa, tên thuốc, ghi chú, milestone… trong toàn bộ dữ liệu của bé.')+
      '</small></div>';
    return;
  }
  var noQuery=!(st.q||'').trim()&&!st.types.size&&st.range==='all';
  var cap=noQuery?60:500;
  var shown=list.slice(0,cap);
  var header=noQuery?'<div class="gsSectionLbl">Gần đây</div>':'';
