...
function babySleepStatusText(db){var latest=latestCareEventByType(db,'sleep');return latest&&!latest.timeTo?'Bé đang ngủ':'Bé đang thức'}
...
function renderDashboard(db){
  ...
  blocks.babyInfo=function(){
    ...
    h+='<div class="bcStatusBar"><div class="bcStatus">😊 '+esc(statusLine())+'</div><div class="bcClock"><span>🕘 <span id="vnClock">--:--:--</span></span><span class="bcTodayDate">'+esc(weekdayDateLine(todayStr))+'</span></div></div>';var sleepStatus=babySleepStatusText(db),nextFeed=nextFeedText(db);h+='<div class="bcStatusExtra"><div><b>Trạng thái:</b> '+esc(sleepStatus)+'</div>'+(nextFeed?'<div><b>Cữ bú tiếp theo:</b> '+esc(nextFeed)+'</div>':'')+'</div>';
    h+='</section>';return h;
  };
