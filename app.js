...
function syncCareFormChromeForType(type){
  /* V15.0.6: Timer chỉ còn đúng một nút "Bắt đầu bú" và chỉ hiện trong form Bé bú.
...
function renderCareTimerState(){var box=byId('careTimerBox');if(!box)return;var selected=window.__careSelectedType||'feed';var t=loadCareTimer();var feedRunning=t&&t.type==='feed';box.classList.toggle('hidden',selected!=='feed'&&!feedRunning);if(selected!=='feed'&&!feedRunning){box.innerHTML='';return}if(!feedRunning){box.innerHTML='<div class="careTimerStarter"><div class="careTimerStarterInfo"><span class="careTimerStarterPill">🍼 Timer bú</span><small>Bắt đầu đếm thời gian ngay trong form Bé bú</small></div><button type="button" class="feedTimerStart" onclick="startCareTimer()" aria-label="Bắt đầu đếm thời gian bé bú"><span>⏱</span><b>Bắt đầu bú</b></button></div>';return}var started=new Date(t.startedAt),sec=Math.max(0,Math.floor((Date.now()-started.getTime())/1000));box.innerHTML='<div class="careTimerRunning careTimerFeedRunning"><div class="careTimerMain"><div class="careTimerHead"><b class="careTimerBadge">🍼 Đang bú</b><small>Từ '+String(started.getHours()).padStart(2,'0')+':'+String(started.getMinutes()).padStart(2,'0')+'</small></div><span>'+fmtHHMMSSDuration(sec)+'</span></div><div class="careTimerBtns"><button type="button" class="ok" onclick="stopCareTimer()">Dừng & điền giờ</button><button type="button" class="danger" onclick="cancelCareTimer()">Hủy</button></div></div>'}
...
function tl8Detail(id){
  ...
  h+='<div class="tl8DetailActionScroller" aria-label="Thao tác bản ghi">'+
       '<button type="button" class="primary" onclick="tl8Edit(\''+esc(id)+'\')" title="Sửa"><span>✏️</span><b>Sửa</b></button>'+
       '<button type="button" onclick="tl8Duplicate(\''+esc(id)+'\')" title="Nhân bản"><span>📄</span><b>Nhân bản</b></button>'+
       '<button type="button" onclick="tl8PickPhoto(\''+esc(id)+'\')" title="Thêm ảnh"><span>📷</span><b>Ảnh</b></button>'+
       '<button type="button" onclick="tl8PickVideo(\''+esc(id)+'\')" title="Thêm video"><span>🎥</span><b>Video</b></button>'+
       '<button type="button" onclick="tl8OpenNote(\''+esc(id)+'\')" title="Ghi chú"><span>📝</span><b>Ghi chú</b></button>'+
       '<button type="button" class="toggle'+favCls+'" onclick="tl8ToggleFav(\''+esc(id)+'\')" title="Yêu thích"><span>⭐</span><b>'+(x.fav?'Bỏ thích':'Yêu thích')+'</b></button>'+
       ...
       '<button type="button" onclick="tl8ExportPdf(\''+esc(id)+'\')" title="Xuất PDF"><span>📄</span><b>PDF</b></button>'+
       '<button type="button" class="danger" onclick="tl8Delete(\''+esc(id)+'\')" title="Xóa"><span>🗑️</span><b>Xóa</b></button>'+
     '</div>';
  h+='</div>';
  body.innerHTML=h;
  tl8Show(ov);
...
/* ============================================================================
   V15.0.6 · FeedStatus TimelineUX — Timer bú + trạng thái + Timeline chip
   ============================================================================ */
(function(){
  var OPEN_SEL=[
    '.tl8Sheet.show','.tl8Overlay.show','.moreSheet.show','.streakOverlay.show',
