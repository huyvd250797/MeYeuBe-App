...
/* V15.0.57 · PumpMilk24UI — Kho sữa là nguồn đúng khi sửa Hút sữa */
...
function evaluateSmartAlerts(db){
  ...
  if(feedRule&&feedRule.enabled!==false){
    ...
    if(latestFeed&&isFinite(grace)&&grace>=0){
      // V15.0.57: Smart Alert theo đúng số phút đã cấu hình sau cữ bú gần nhất.
...
function renderDashboard(db){
  ...
  blocks.babyInfo=function(){
    ...
    /* V15.0.57: Ngày sinh / thông tin lúc sinh chuyển sang modal chi tiết bé, dashboard không hiển thị nữa. */
...
/* V15.0.57 · QuietCloudToastFix — không render DB rỗng trước khi Cloud DB kéo xong */
...
/* ============================================================================
   V15.0.57 · MilkLedgerFix — ledger kho sữa, không hồi sinh túi quá hạn/đã hủy
   ============================================================================ */
(function(){
...
/* ============================================================================
   V15.0.57 · SmartAlertCronPush — mỗi lần Hút sữa sở hữu bình/túi riêng
   ============================================================================ */
(function(){
...
/* ============================================================================
   V15.0.57 · PIN Data Guard — bảo vệ Cloud Sync + Dữ liệu/Backup
   ============================================================================ */
(function(){
...
/* ============================================================================
   V15.0.57 · StoredFeedFastAutoFix — sửa Bé bú từ kho tự co/giãn túi theo ml
   ============================================================================ */
(function(){
  function N(v){v=Number(v||0);return isFinite(v)?Math.max(0,Math.round(v)):0}
  function S(v){return String(v==null?'':v)}
  ...
  window.fillCareEditForm=fillCareEditForm=function(i){
    ...
    }catch(e){console.warn('V15.0.57 fill edit auto mode failed',e)}
    return r;
  };
...
/* ============================================================================
   V15.0.57 · StoredFeedFastAutoFix — Bé bú từ kho chỉnh ml nhanh, chỉ ✕ mới thủ công
   ============================================================================ */
(function(){
  function N(v){v=Number(v||0);return isFinite(v)?Math.max(0,Math.round(v)):0}
  function S(v){return String(v==null?'':v)}
...
/* ============================================================================
   V15.0.57 · BabyProfileModalUX — thông tin bé + khóa scroll + điều hướng an toàn
   ============================================================================ */
(function(){
...
/* ============================================================================
   V15.0.57 · HealthDocsNavFix — hồ sơ giấy tờ + sidebar/taskbar scroll
   ============================================================================ */
(function(){
...
/* ============================================================================
   V15.0.57 · SupabaseCloudDBMode — khóa an toàn kho sữa + scroll + hồ sơ
   ============================================================================ */
(function(){
  ...
  window.normalize=normalize=function(db){db=baseNormalize(db);try{window.recalculateMilkInventoryLedger(db,{quiet:true})}catch(e){console.warn('Milk ledger normalize V15.0.57',e)}return db};
...
/* ============================================================================
   V15.0.57 · SupabaseCloudDBMode — navbar scroll + PIN ngân hàng + file hồ sơ
   ============================================================================ */
(function(){
  function S(v){return String(v==null?'':v)}
...
/* ============================================================================
   V15.0.57 · SupabaseCloudDBMode — giảm tải localStorage, lưu tệp hồ sơ vào IndexedDB
   ============================================================================ */
(function(){
  ...
  window.dataGuardBackup=dataGuardBackup=function(db,reason){
    try{
      var snap={reason:reason||'auto',at:new Date().toISOString(),counts:(typeof dataCountSnapshot==='function'?dataCountSnapshot(db):{}),dbBytes:roughSize(db),note:'V15.0.57 chỉ lưu metadata snapshot để tránh đầy localStorage'};
...
/* V15.0.57 · CloudDB guard for theme save — tránh ghi DB chính vào localStorage khi đổi dark/light */
(function(){
...
(function(){
  ...
  function safeMerge1545(remote,local,tag){
    ...
    out._cloudMergeGuard='V15.0.57 QuietCloudToastFix';
...
(function(){
  ...
  function mergeSafe(remote,local,tag){var r=norm(remote||{}),l=norm(local||{}),ts=trimTs(tombMap(r,l)),out=mergeObj(r,l);paths(r,l).forEach(function(p){out[p]=mergeArr(p,r[p],l[p],ts)});out._sync=obj(out._sync)?out._sync:{};out._sync.tombstones=ts;out._sync.ops=mergeOps(r,l);out._sync.schema='cloud-db-oplog-v1';out._sync.lastMergedAt=now();out._sync.lastMergedBy=dev();out._sync.lastMergeReason=tag||'merge';out._cloudRevision=Math.max(Number(r._cloudRevision||0),Number(l._cloudRevision||0));out._cloudMergeGuard='V15.0.57 QuietCloudToastFix';out._cloudMergeGuardAt=now();out._cloudMergeGuardReason=tag||'merge';try{return normalize(out)}catch(e){return out}}
...
/* V15.0.57 · QuietCloudToastFix — gom toast Cloud/merge lúc khởi động, chỉ báo “Đã kết nối” khi ổn */
(function(){
...
/* ============================================================================
   V15.0.57 · StoredFeedInventoryLinkFix — khôi phục auto chọn kho sữa ổn định
...
(function(){
  ...
  window.fillCareEditForm=fillCareEditForm=function(i){
    ...
    try{var db=dbNow(),x=(db.careEvents||[])[Number(i)];if(x&&x.type==='feed'&&x.source==='stored'){
      ...
      if(byId('cAmount'))setValSafe('cAmount',taken);
      try{renderMilkSourceList();updateCareMilkSourceTotal();abSyncChrome()}catch(e){}
    }}catch(e){console.warn('V15.0.57 fill stored feed source failed',e)}
    return r;
  };
})();

/* ============================================================================
   V15.0.57 · StartupLoadingWatchdogFix — chống kẹt loading khi Cloud DB chậm
...
/* ============================================================================
   V15.0.57 · CloudRealtimeAuthoritativeFix — chống Sổ sức khỏe biến Ba/Mẹ thành nhiều Bé
...
(function(){
  ...
  window.cloudMergePayloads=cloudMergePayloads=function(remote,local){
    ...
    }catch(e){console.warn('CloudRealtimeAuthoritativeFix merge failed',e);return out}
  };
...
/* ============================================================================
   V15.0.57 · CloudRealtimeAuthoritativeFix — chuẩn hóa quyền ghi Cloud DB
...
(function(){
  ...
  var TRACK_SECTIONS=['settings','settings.dashboardConfig','settings.babyDescription','settings.showOfficialName','hb','hb.members','healthBook','baby','mom','milkContainers','appointmentTypes','diaryTypes','monthlyNotes'];
  ...
  function sectionMerge(remote,local,tag){
    ...
    base._cloudMergeGuard='V15.0.57 CloudRealtimeAuthoritativeFix';base._cloudMergeGuardAt=now();base._cloudMergeGuardReason=tag||'section_merge';
    return norm(base);
  }
  ...
  function setMem(dbObj,source){var n=norm(dbObj||{});n._cloudDbMode=true;n._cloudDbSource=source||'v1554';window.__mybCloudDbMemory=n;try{putCache(n).catch(function(e){console.warn('V15.0.57 cache put failed',e)})}catch(e){}return n}
  ...
  window.save=save=function(dbObj){
    ...
    try{putCache(next).catch(function(e){console.warn('V15.0.57 pre-save cache failed',e)})}catch(e){}
  ...
  window.cloudAutoPullOnBoot=cloudAutoPullOnBoot=async function(){
    ...
    try{cached=await Promise.race([getCache(),new Promise(function(resolve){setTimeout(function(){resolve(null)},650)})]);if(cached)setMem(cached,'startup_cache_first_v1554')}catch(e){console.warn('V15.0.57 cache boot failed',e)}
    try{if(navigator.onLine){row=await cloudFetchRow(c);remote=row&&row.payload?norm(row.payload):null}}catch(e){console.warn('V15.0.57 cloud boot failed',e);if(cached)return 'cache'}
    if(remote){var merged=sectionMerge(remote,cached||norm(load()),'startup_cloud_cache_merge_v1554');setMem(merged,'startup_merged_v1554');try{if(score(merged)>=score(remote)&&stable(merged)!==stable(remote)){cloudUpsertPayload(c,merged).catch(function(e){console.warn('V15.0.57 startup pushback failed',e)})}}catch(e){};try{render()}catch(e){};return 'cloud-merged'}
    if(cached){try{cloudUpsertPayload(c,cached).catch(function(e){console.warn('V15.0.57 startup cache push failed',e)})}catch(e){};return 'cache-pushed'}
...
/* V15.0.57 · CloudRealtimeAuthoritativeFix
   ...
   - Mỗi nhóm dữ liệu có section stamp; remote cũ không được đè section mới.
...
(function(){
  ...
  var nativeCloudApply=window.cloudApplyRemotePayload||cloudApplyRemotePayload;
  var nativeCloudAutoPull=window.cloudAutoPullOnBoot||cloudAutoPullOnBoot;
  var nativePushLocal=window.pushLocalToCloud||pushLocalToCloud;
  var nativePullCloud=window.pullCloudToLocal||pullCloudToLocal;
  var nativeSmartSync=window.smartCloudSync||smartCloudSync;
  var nativeCloudUpsert=window.cloudUpsertPayload||cloudUpsertPayload;

  var SECTIONS=[
    'settings','hb','healthBook','baby','mom','careEvents','milkInventory','appointments','milestones','diary',
    'appointmentTypes','diaryTypes','milkContainers','monthlyNotes','pregnancy','noiseLogs','luxLogs'
  ];
  var IMPORTANT=['settings','hb','healthBook','baby','mom'];
  var CACHE_DB='meYeuBeCloudDb_v1',CACHE_STORE='state',CACHE_KEY='main';
  var pending=null,saving=false,queued=false,timer=null;

  function mode(){try{return typeof cloudDbModeEnabled==='function'&&cloudDbModeEnabled()}catch(e){return false}}
  function dev(){try{return cloudDeviceId()}catch(e){return 'local'}}
  function now(){return new Date().toISOString()}
  function clone(v){try{return JSON.parse(JSON.stringify(v||{}))}catch(e){return {}}}
  function norm(v){try{return normalize(clone(v||{}))}catch(e){return clone(v||{})}}
  ...
  function stable(v){
    ...
    if(typeof v!=='object')return JSON.stringify(v);
    if(Array.isArray(v))return '['+v.map(stable).join(',')+']';
    var keys=Object.keys(v).filter(function(k){return k!=='_swipeOpen'&&k!=='_idx'&&k!=='_key'}).sort();
    return '{'+keys.map(function(k){return JSON.stringify(k)+':'+stable(v[k])}).join(',')+'}';
  }
  function score(db){try{return dataSnapshotScore(dataCountSnapshot(db))}catch(e){try{return JSON.stringify(db||{}).length}catch(_e){return 0}}}
  function sync(db){db._sync=isObj(db._sync)?db._sync:{};db._sync.schema='cloud-db-authority-v2';db._sync.sectionStamps=isObj(db._sync.sectionStamps)?db._sync.sectionStamps:{};return db._sync}
  function maxSeq(){var m=0;for(var i=0;i<arguments.length;i++){var db=arguments[i]||{},s=(db._sync&&db._sync.sectionStamps)||{};m=Math.max(m,Number(db._sync&&db._sync.commitSeq||0),Number(db._localCommitSeq||0),Number(db._cloudRevision||0));Object.keys(s).forEach(function(k){m=Math.max(m,Number((s[k]&&s[k].seq)||0))})}return m}
  function sectionVal(db,k){return (db||{})[k]}
  function sectionStamp(db,k){
    db=db||{};var s=(db._sync&&db._sync.sectionStamps)||{},st=s[k]||{};
    var seq=Number(st.seq||0);if(seq>0)return seq;
    if(k==='hb')return Number(db._hbLocalCommitSeq||0)||0;
    if(k==='settings')return Number(db._settingsLocalCommitSeq||0)||0;
    return 0;
  }
  function sectionTime(db,k){
    db=db||{};var s=(db._sync&&db._sync.sectionStamps)||{},st=s[k]||{};
    return Date.parse(st.at||db._localUpdatedAt||db._cloudUpdatedAt||0)||0;
  }
  function changedSections(before,next){
    before=before||{};next=next||{};var arr=[];
    SECTIONS.forEach(function(k){try{if(stable(sectionVal(before,k))!==stable(sectionVal(next,k)))arr.push(k)}catch(e){}});
    if(!arr.length)arr=['_db'];
    return arr;
  }
  function markCommit(next,before,reason){
    next=norm(next||{});before=norm(before||{});
    var changed=changedSections(before,next),seq=maxSeq(before,next)+1,at=now(),s=sync(next),prev=(before._sync&&before._sync.sectionStamps)||{};
    Object.keys(prev).forEach(function(k){if(!s.sectionStamps[k])s.sectionStamps[k]=clone(prev[k])});
    changed.forEach(function(k){s.sectionStamps[k]={seq:seq,at:at,deviceId:dev(),reason:reason||'save'}});
    s.commitSeq=Math.max(Number(s.commitSeq||0),seq);s.lastLocalCommitAt=at;s.lastLocalCommitBy=dev();s.lastChangedSections=changed.slice();
    next._localUpdatedAt=at;next._localCommitSeq=seq;next._localCommitDeviceId=dev();next._localCommitId='lc_'+seq+'_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,6);
    if(changed.indexOf('hb')>=0){next._hbLocalCommitAt=at;next._hbLocalCommitSeq=seq;}
    if(changed.indexOf('settings')>=0){next._settingsLocalCommitAt=at;next._settingsLocalCommitSeq=seq;}
    pending={id:next._localCommitId,at:at,seq:seq,sections:changed.slice(),db:clone(next),until:Date.now()+45000};
    window.__mybLastAuthoritativeLocalCommit=pending;
    return next;
  }
  function openCache(){return new Promise(function(resolve,reject){if(!('indexedDB' in window)){reject(new Error('Không hỗ trợ IndexedDB'));return}var r=indexedDB.open(CACHE_DB,1);r.onupgradeneeded=function(){var db=r.result;if(!db.objectStoreNames.contains(CACHE_STORE))db.createObjectStore(CACHE_STORE,{keyPath:'key'})};r.onsuccess=function(){resolve(r.result)};r.onerror=function(){reject(r.error||new Error('Không mở được cache'))}})}
  function putCache(dbObj){return openCache().then(function(db){return new Promise(function(resolve,reject){var tx=db.transaction(CACHE_STORE,'readwrite'),st=tx.objectStore(CACHE_STORE);st.put({key:CACHE_KEY,data:norm(dbObj||{}),updatedAt:now()});tx.oncomplete=function(){try{db.close()}catch(e){};resolve(true)};tx.onerror=function(){try{db.close()}catch(e){};reject(tx.error||new Error('Không ghi cache'))}})})}
  function getCache(){return openCache().then(function(db){return new Promise(function(resolve,reject){var tx=db.transaction(CACHE_STORE,'readonly'),st=tx.objectStore(CACHE_STORE),r=st.get(CACHE_KEY);r.onsuccess=function(){try{db.close()}catch(e){};resolve(r.result&&r.result.data?norm(r.result.data):null)};r.onerror=function(){try{db.close()}catch(e){};reject(r.error||new Error('Không đọc cache'))}})})}
  function setAuthoritative(dbObj,source){
    ...
    try{putCache(n).catch(function(e){console.warn('V15.0.57 cache put failed',e)})}catch(e){}
    return n;
  }
  function richer(a,b){return score(a)>=score(b)?a:b}
  function mergeObjectsRemoteLocal(remoteObj,localObj,localWins){
    remoteObj=isObj(remoteObj)?remoteObj:{};localObj=isObj(localObj)?localObj:{};
    var out=clone(remoteObj),keys={};Object.keys(remoteObj).forEach(function(k){keys[k]=1});Object.keys(localObj).forEach(function(k){keys[k]=1});
    Object.keys(keys).forEach(function(k){
      var rv=remoteObj[k],lv=localObj[k];
      if(Array.isArray(rv)||Array.isArray(lv)){out[k]=localWins?(lv!==undefined?clone(lv):clone(rv)):(rv!==undefined?clone(rv):clone(lv));return}
      if(isObj(rv)||isObj(lv)){out[k]=mergeObjectsRemoteLocal(rv,lv,localWins);return}
      if(localWins)out[k]=(lv!==undefined&&lv!==null&&lv!=='')?lv:rv;
      else out[k]=(rv!==undefined&&rv!==null&&rv!=='')?rv:lv;
    });
    return out;
  }
  function sectionLocalProtected(k,local,remote){
    if(pending&&pending.until>Date.now()&&pending.sections&&pending.sections.indexOf(k)>=0)return true;
    var ls=sectionStamp(local,k),rs=sectionStamp(remote,k);
    if(ls>rs)return true;
    return false;
  }
  function mergeAuthority(remote,local,tag){
    var r=norm(remote||{}),l=norm(local||{}),base;
    try{base=(typeof window.mybCloudSafeMergeV1546==='function')?window.mybCloudSafeMergeV1546(r,l,tag||'authority_v1555'):cloudMergePayloads(r,l)}catch(e){base=Object.assign({},clone(r),clone(l))}
    base=norm(base||{});
    var bs=sync(base),rs=(r._sync&&r._sync.sectionStamps)||{},ls=(l._sync&&l._sync.sectionStamps)||{};
    Object.keys(rs).forEach(function(k){if(!bs.sectionStamps[k])bs.sectionStamps[k]=clone(rs[k])});
    Object.keys(ls).forEach(function(k){
      var a=bs.sectionStamps[k]||{},b=ls[k]||{};
      if(Number(b.seq||0)>=Number(a.seq||0))bs.sectionStamps[k]=clone(b);
    });
    SECTIONS.forEach(function(k){
      var rv=r[k],lv=l[k],rsq=sectionStamp(r,k),lsq=sectionStamp(l,k);
      if(sectionLocalProtected(k,l,r)){base[k]=clone(lv);return}
      if(rsq>lsq){base[k]=clone(rv);return}
      if(lsq>rsq){base[k]=clone(lv);return}
      // Legacy/no stamp: for critical object sections keep the richer/local side, never let empty stale cloud erase visible data.
      if(IMPORTANT.indexOf(k)>=0){
        if(stable(lv).length>=stable(rv).length)base[k]=mergeObjectsRemoteLocal(rv,lv,true);
        else base[k]=mergeObjectsRemoteLocal(rv,lv,false);
      }
    });
    bs.commitSeq=Math.max(maxSeq(base,r,l),Number(bs.commitSeq||0));
    base._cloudMergeGuard='V15.0.57 CloudRealtimeAuthoritativeFix';
    base._cloudMergeGuardAt=now();
    base._cloudMergeGuardReason=tag||'authority_merge';
    return norm(base);
  }
  async function restPatchOrPost(cfg,prepared,row){
    var headers=Object.assign({},cloudHeaders(cfg),{'Prefer':'resolution=merge-duplicates,return=representation'}),base=cloudEndpoint(cfg),t=prepared._cloudUpdatedAt;
    if(row&&row.updatedAt){
      try{
        var res=await cloudRequestJson(base+'?id=eq.'+encodeURIComponent(cfg.syncId)+'&updated_at=eq.'+encodeURIComponent(row.updatedAt),{method:'PATCH',headers:Object.assign({},cloudHeaders(cfg),{'Prefer':'return=representation'}),body:JSON.stringify({data:prepared,updated_at:t})},'Cloud CAS update');
        if(Array.isArray(res)&&res.length)return res;
        if(res&&(!Array.isArray(res)))return res;
        return [];
      }catch(e){
        if(!cloudIsMissingColumnError(e,'id')&&!cloudIsMissingColumnError(e,'data'))throw e;
        var res2=await cloudRequestJson(base+'?sync_id=eq.'+encodeURIComponent(cfg.syncId)+'&updated_at=eq.'+encodeURIComponent(row.updatedAt),{method:'PATCH',headers:Object.assign({},cloudHeaders(cfg),{'Prefer':'return=representation'}),body:JSON.stringify({payload:prepared,updated_at:t})},'Cloud CAS update legacy');
        return res2;
      }
    }
    try{return await cloudRequestJson(base,{method:'POST',headers:headers,body:JSON.stringify({id:cfg.syncId,data:prepared,updated_at:t})},'Cloud upsert')}
    catch(e){if(!cloudIsMissingColumnError(e,'id')&&!cloudIsMissingColumnError(e,'data'))throw e;return await cloudRequestJson(base,{method:'POST',headers:headers,body:JSON.stringify({sync_id:cfg.syncId,payload:prepared,updated_at:t})},'Cloud upsert legacy')}
  }
  function prepareCloudPayload(dbObj,cfg){
    var out=norm(dbObj||{}),seq=maxSeq(out)+1;
    out._cloudRevision=Math.max(Number(out._cloudRevision||0),Number((cfg&&cfg.lastRevision)||0),seq);
    out._cloudDeviceId=dev();out._cloudUpdatedAt=now();out._cloudDbMode=true;
    var s=sync(out);s.lastCommitAt=out._cloudUpdatedAt;s.lastCommitDeviceId=dev();s.commitSeq=Math.max(Number(s.commitSeq||0),out._cloudRevision,Number(out._localCommitSeq||0));
    return out;
  }
  async function upsertAuthority(cfg,payload){
    cloudValidateCfg(cfg);var local=norm(payload||load()),attempt=0;
    while(attempt<4){
      attempt++;
      var row=null,remote=null;
      try{row=await cloudFetchRow(cfg);remote=row&&row.payload?norm(row.payload):null}catch(e){console.warn('V15.0.57 fetch before commit failed',e)}
      ...
      if(row&&row.updatedAt&&Array.isArray(res)&&res.length===0){
        local=mergeAuthority(prepared,local,'cas_retry_local_v1555_'+attempt);
        continue;
      }
      cfg.lastRevision=Math.max(Number(cfg.lastRevision||0),Number(prepared._cloudRevision||0));cfg.lastPushedAt=prepared._cloudUpdatedAt;saveCloudConfigToStorage(cfg);
      setAuthoritative(prepared,'cloud_commit_ack_v1555');
      if(pending&&pending.id&&prepared._localCommitId===pending.id)pending=null;
      return {result:res,payload:prepared};
    }
    throw new Error('Cloud commit bận, vui lòng thử lại sau');
  }
  function scheduleFlush(reason,ms){clearTimeout(timer);timer=setTimeout(function(){flushAuthority(reason||'scheduled').catch(function(e){console.error('V15.0.57 flush failed',e)})},ms==null?180:ms)}
  async function flushAuthority(reason){
    ...
    try{var cfg=loadCloudConfig(),local=norm(window.__mybCloudDbMemory||load());var pushed=await upsertAuthority(cfg,local);try{renderCloudConfig()}catch(e){};return !!pushed}
    finally{saving=false;if(queued){queued=false;scheduleFlush('queued_v1555',300)}}
  }

  window.cloudMergePayloads=cloudMergePayloads=function(remote,local){if(!mode())return mergeAuthority(remote,local,'merge_non_mode_v1555');return mergeAuthority(remote,local,'cloudMergePayloads_v1555')};
  window.cloudUpsertPayload=cloudUpsertPayload=async function(cfg,payload){if(!mode())return nativeCloudUpsert(cfg,payload);return upsertAuthority(cfg,payload)};
  window.cloudDbFlush=flushAuthority;

  window.save=save=function(dbObj){
    if(!mode())return nativeSave(dbObj);
    var before=norm(load()),next=markCommit(dbObj,before,'local_save_v1555');
    try{pruneAutoMilestones(next)}catch(e){}try{checkAutoMilestones(next)}catch(e){}
    setAuthoritative(next,'local_save_v1555');
    try{render()}catch(e){}
    scheduleFlush('local_save_v1555',120);
  ...
  window.cloudApplyRemotePayload=cloudApplyRemotePayload=function(payload,updatedAt,source){
    if(!payload)return false;
    try{if(payload._cloudDeviceId===dev())return false}catch(e){}
    if(!mode())return nativeCloudApply(payload,updatedAt,source);
    var local=norm(load());
    var remote=norm(payload);
    var next=mergeAuthority(remote,local,'realtime_apply_v1555');
    setAuthoritative(next,'realtime_merge_v1555');
    try{var c=loadCloudConfig();c.lastPulledAt=now();c.lastRevision=Math.max(Number(c.lastRevision||0),Number(next._cloudRevision||0),Number(remote._cloudRevision||0));saveCloudConfigToStorage(c)}catch(e){}
    try{render()}catch(e){}
    if(pending&&pending.until>Date.now())scheduleFlush('realtime_protected_pushback_v1555',260);
    return true;
  };
  ...
  window.cloudAutoPullOnBoot=cloudAutoPullOnBoot=async function(){
    ...
    try{cached=await Promise.race([getCache(),new Promise(function(resolve){setTimeout(function(){resolve(null)},700)})]);if(cached)setAuthoritative(cached,'startup_cache_v1555')}catch(e){console.warn('V15.0.57 cache boot failed',e)}
    try{if(navigator.onLine){row=await cloudFetchRow(cfg);remote=row&&row.payload?norm(row.payload):null}}catch(e){console.warn('V15.0.57 cloud boot failed',e);return cached?'cache':'cloud-error'}
    if(remote){
      var base=cached||norm(load());
      var merged=mergeAuthority(remote,base,'startup_cloud_cache_merge_v1555');
      setAuthoritative(merged,'startup_merged_v1555');
      try{render()}catch(e){}
      // Nếu cache/local có section mới hơn Cloud, đẩy lại để Cloud không giữ bản cũ.
      try{if(stable(merged)!==stable(remote))scheduleFlush('startup_pushback_v1555',300)}catch(e){}
      return 'cloud-merged';
    }
    if(cached){scheduleFlush('startup_cache_first_push_v1555',300);return 'cache-pushed'}
    return nativeCloudAutoPull.apply(this,arguments);
  };

  window.pushLocalToCloud=pushLocalToCloud=async function(){
    if(!mode())return nativePushLocal.apply(this,arguments);
    if(!confirm('Gộp & đẩy dữ liệu hiện tại lên Supabase Cloud DB?\n\nApp sẽ fetch Cloud mới nhất trước, giữ dữ liệu mới vừa lưu rồi mới commit.'))return;
    try{showAppLoading();await flushAuthority('manual_push_v1555');showToast('Đã đẩy Cloud DB an toàn','success')}catch(e){showToast('Đẩy Cloud DB thất bại: '+(e.message||e),'error')}finally{hideAppLoading();try{renderCloudConfig()}catch(e){}}
  };
  window.pullCloudToLocal=pullCloudToLocal=async function(){
    if(!mode())return nativePullCloud.apply(this,arguments);
    if(!confirm('Gộp dữ liệu từ Supabase về app?\n\nBản này không thay thế một chiều và không làm mất dữ liệu vừa sửa.'))return;
    try{showAppLoading();var c=loadCloudConfig(),row=await cloudFetchRow(c);if(!row||!row.payload){showToast('Cloud chưa có dữ liệu','warn');return}var merged=mergeAuthority(row.payload,load(),'manual_pull_v1555');setAuthoritative(merged,'manual_pull_v1555');try{render()}catch(e){};scheduleFlush('manual_pull_pushback_v1555',220);showToast('Đã gộp Cloud DB an toàn','success')}catch(e){showToast('Gộp Cloud DB thất bại: '+(e.message||e),'error')}finally{hideAppLoading();try{renderCloudConfig()}catch(e){}}
  };
  window.smartCloudSync=smartCloudSync=async function(){
    if(!mode())return nativeSmartSync.apply(this,arguments);
    try{showAppLoading();await cloudAutoPullOnBoot();await flushAuthority('smart_sync_v1555');showToast('Đã đồng bộ Cloud DB an toàn','success')}catch(e){showToast('Đồng bộ Cloud DB thất bại: '+(e.message||e),'error')}finally{hideAppLoading();try{renderCloudConfig()}catch(e){}}
  };
})();
...
(function(){
  if(window.__MYB_HEALTH_BOOK_NORMALIZE_GUARD_V1556__)return;
  window.__MYB_HEALTH_BOOK_NORMALIZE_GUARD_V1556__=true;
  ...
  function safeHealthBookRows(v){
    ...
    if(isObj(v)){
      ...
      // Có thiết bị/bản cũ từng ghi nhầm healthBook={members:[...]}; members thuộc hb, không phải legacy healthBook.
      return [];
    }
    return [];
  }
  function coerce(db){
    db=isObj(db)?clone(db):{};
    db.healthBook=safeHealthBookRows(db.healthBook);
    if(Array.isArray(db.hb))db.hb={members:db.hb};
    db.hb=isObj(db.hb)?db.hb:{};
    db.hb.members=arr(db.hb.members).map(function(m){return isObj(m)?m:{}});
    ['pregnancy','baby','mom','diary','appointments','milestones','careEvents','milkInventory','noiseLogs','luxLogs'].forEach(function(k){if(!Array.isArray(db[k]))db[k]=[]});
    if(!isObj(db.settings))db.settings={};
    if(!isObj(db.monthlyNotes))db.monthlyNotes={};
    return db;
  }
  function fallbackNormalize(db,e){
    var out=coerce(db);
    try{out.settings=out.settings||{};out.healthBook=safeHealthBookRows(out.healthBook);out.hb=out.hb||{};out.hb.members=arr(out.hb.members);return out}catch(_e){return {settings:{},healthBook:[],hb:{members:[]},careEvents:[],milkInventory:[],appointments:[],milestones:[],diary:[],baby:[],mom:[],pregnancy:[]}}
  }
  window.mybCoerceHealthBookShapeV1556=coerce;
  window.normalize=normalize=function(db){
    var shaped=coerce(db);
    try{return nativeNormalize(shaped)}
    catch(e){
      console.warn('HealthBookNormalizeGuard V15.0.57 fallback',e);
      try{showToast&&showToast('Đã tự sửa cấu trúc Sổ sức khỏe cũ để tải dữ liệu','warn')}catch(_e){}
      return fallbackNormalize(shaped,e);
    }
  };
  window.cloudMergePayloads=cloudMergePayloads=function(remote,local){
    try{return nativeCloudMerge(coerce(remote),coerce(local))}
    catch(e){console.warn('HealthBookNormalizeGuard cloud merge fallback',e);var r=coerce(remote),l=coerce(local);return normalize(Object.assign({},r,l,{hb:(l.hb&&l.hb.members&&l.hb.members.length)?l.hb:r.hb}))}
  };
  window.cloudApplyRemotePayload=cloudApplyRemotePayload=function(payload,updatedAt,source){
    try{return nativeCloudApply(coerce(payload),updatedAt,source)}
    catch(e){console.warn('HealthBookNormalizeGuard realtime fallback',e);try{showToast('Không áp dụng Cloud cũ vì sai cấu trúc Sổ sức khỏe. App giữ dữ liệu hiện tại.','warn')}catch(_e){};return false}
  };
  window.cloudAutoPullOnBoot=cloudAutoPullOnBoot=async function(){
    try{return await nativeCloudAutoPull.apply(this,arguments)}
    catch(e){
      console.warn('HealthBookNormalizeGuard boot fallback',e);
      try{hideAppLoading&&hideAppLoading();hideSplashScreen&&hideSplashScreen()}catch(_e){}
      try{showToast('Cloud DB có dữ liệu Sổ sức khỏe cũ sai cấu trúc, app đã mở cache an toàn','warn')}catch(_e){}
      try{render&&render()}catch(_e){}
      return 'healthbook-shape-fallback';
    }
  };
})();
...
/* V15.0.57 · HealthBookActiveMemberFix
   ...
   Fix: active member là trạng thái UI cục bộ theo thiết bị, không xem là dữ liệu Cloud authoritative. */
(function(){
  if(window.__MYB_HB_ACTIVE_MEMBER_FIX_V1557__)return;
  window.__MYB_HB_ACTIVE_MEMBER_FIX_V1557__=true;
  var KEY='myb_hb2_active_member_id_v1557';
  function S(v){return String(v==null?'':v)}
  function A(v){return Array.isArray(v)?v:[]}
  function esc2(v){try{return esc(v)}catch(e){return S(v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}}
  function js(v){return S(v).replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/\n/g,'\\n').replace(/\r/g,'')}
  function dbMembers(db){try{return hb2Members(db)}catch(e){return A(db&&db.hb&&db.hb.members)}}
  function exists(db,id){id=S(id);return !!dbMembers(db).some(function(m){return S(m&&m.id)===id})}
  function firstId(db){var m=dbMembers(db)[0];return m&&m.id?S(m.id):''}
  function readLocal(){try{return localStorage.getItem(KEY)||''}catch(e){return ''}}
  function writeLocal(id){try{if(id)localStorage.setItem(KEY,S(id));else localStorage.removeItem(KEY)}catch(e){}}
  function resolveActiveId(db){
    db=db||{};
    var sid=S(window.hb2State&&window.hb2State.activeId);
    if(sid&&exists(db,sid))return sid;
    var lid=readLocal();
    if(lid&&exists(db,lid)){try{hb2State.activeId=lid}catch(e){};return lid;}
    var did=S(db&&db.hb&&db.hb.activeId);
    if(did&&exists(db,did)){try{hb2State.activeId=did}catch(e){};writeLocal(did);return did;}
    var fid=firstId(db);
    if(fid){try{hb2State.activeId=fid}catch(e){};writeLocal(fid);}
    return fid;
  }
  function findMember(db,id){id=S(id);var arr=dbMembers(db);for(var i=0;i<arr.length;i++){if(S(arr[i]&&arr[i].id)===id)return arr[i]}return arr[0]||null}
  window.hb2ResolveActiveId=resolveActiveId;
  window.hb2Active=hb2Active=function(db){return findMember(db,resolveActiveId(db))};
  window.hb2SetActive=hb2SetActive=function(id){
    var db=load();id=S(id);
    if(!exists(db,id)){try{showToast('Không tìm thấy hồ sơ đã chọn','warn')}catch(e){};return false;}
    try{hb2State.activeId=id;hb2State.view='home'}catch(e){}
    writeLocal(id);
    try{hb2Render()}catch(e){console.error(e)}
    return false;
  };
  window.hb2ChipClick=hb2ChipClick=function(id){
    var db=load();id=S(id);
    // Click chip chỉ đổi hồ sơ đang xem, không save DB/không đẩy Cloud để tránh realtime kéo activeId cũ đè lại.
    return hb2SetActive(id);
  };
  window.hb2ChipsHtml=hb2ChipsHtml=function(db){
    var arr=dbMembers(db),act=resolveActiveId(db);
    return '<div class="card hb2Head"><div class="hb2HeadTop">'+
      '<h2>🩺 Sổ sức khỏe <span class="hb2Ver">2.0</span></h2>'+
      '<button class="hb2QuickBtn" onclick="hb2OpenQuickAdd()">＋ Thêm nhanh</button>'+
      '</div><div class="hb2Chips">'+
      arr.map(function(m){
        var id=S(m&&m.id),name=S((m&&m.name)||((m&&m.rel)||'Hồ sơ'));
        return '<button type="button" class="hb2Chip'+(id===act?' on':'')+'" onclick="return hb2ChipClick(\''+js(id)+'\')">'+
          '<span class="hb2Ava">'+esc2((m&&m.avatar)||'🧑')+'</span><span class="hb2ChipName">'+esc2(name)+'</span></button>';
      }).join('')+
      '<button type="button" class="hb2Chip add" onclick="hb2OpenAddMember()"><span class="hb2Ava">＋</span><span class="hb2ChipName">Thêm</span></button>'+
      '</div></div>';
  };
  var nativeCommit=window.hb2Commit||hb2Commit;
  window.hb2Commit=hb2Commit=function(db,msg){
    db=db||load();
    var id=resolveActiveId(db),m=findMember(db,id);
    if(m){var t=new Date().toISOString();m.updatedAt=t;m._hbLocalCommitAt=t;}
    // Không đổi activeId chỉ vì lưu/merge. active member đã được giữ bằng local UI state.
    save(db);
    if(msg)try{showToast(msg,'success')}catch(e){}
    try{hb2Render()}catch(e){console.error(e)}
    return true;
  };
  // Khi render Sổ sức khỏe sau Cloud/realtime, khôi phục chip đang xem từ local state.
  try{var db0=load();resolveActiveId(db0)}catch(e){}
})();
