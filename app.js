...
/* ===================== 🍼 V11.4.0 · Giao diện modal chi tiết chăm sóc ===================== */
...
function careNoteChipHtml(x){
  if(!x||!x.note)return '';
  var bottle=(x.type==='feed'&&x.source&&x.source!=='direct');
  var dot=careNoteDotColor(x.note);
  return '<div class="careRecNote'+(bottle?' bottle':'')+'"><small>'+(bottle?'🍼 Ghi chú bình:':'📝 Ghi chú:')+'</small><b>'+esc(x.note)+'</b>'+(dot?'<i class="careRecNoteDot" style="background:'+dot+'"></i>':'')+'</div>';
}
