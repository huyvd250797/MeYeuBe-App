...
function ccxFsRelayout(){
  ...
  window.__ccxFsRz=setTimeout(function(){ccxFsDraw(t)},160);
}
window.addEventListener('resize',ccxFsRelayout);
window.addEventListener('orientationchange',ccxFsRelayout);
function ccxFsClose(){var fs=byId('ccxFs');if(fs)fs.classList.remove('show');document.body.classList.remove('ccxFsOpen');if(window.CCX)window.CCX.fsType=null;}
...
function gsFilter(){
  ...
  var out=exact.length?exact:(fuzzy.length?fuzzy:partial);
  var approx=(!exact.length&&out.length>0);
  ...
  return {list:out,tokens:tokens,approx:approx};
}
...
function gsRender(){
  ...
  var countEl=byId('gsCount');if(countEl)countEl.textContent=list.length+' kết quả'+(res.approx?' gần đúng':'');
  if(!list.length){
  ...
  if(res.approx)header='<div class="gsApproxLbl">🔎 Không có kết quả khớp hoàn toàn — đang hiển thị dữ liệu gần đúng nhất</div>'+header;
