...
function toggleMilestoneMenu(event){
  if(event&&event.preventDefault)event.preventDefault();
  var parent=byId('milestoneParent');
  var sub=byId('milestoneSubNav');
  if(!parent||!sub)return;
  var willOpen=!sub.classList.contains('open');
  sub.classList.toggle('open',willOpen);
  parent.classList.toggle('milestoneOpen',willOpen);
}
function openMilestoneMenu(){
  var parent=byId('milestoneParent');
  var sub=byId('milestoneSubNav');
  if(parent)parent.classList.add('milestoneOpen');
  if(sub)sub.classList.add('open');
}
...
function careDetailSummaryHtml(db,type,date,arr){
  ...
  if(type==='feed'){
    ...
    var wasted=arr.reduce(function(t,x){return t+Number(x.wasteMl||0)},0);
    extra=' · '+direct+' bú trực tiếp · '+stored+' bú kho · '+formula+' sữa công thức'+(wasted>0?' · bỏ tổng '+wasted+'ml':'');
  }
  if(type==='pump'){
...
function babySleepElapsedMinutes(db){
  var latest=latestCareEventByType(db,'sleep');
  if(!latest||latest.timeTo)return null;
  var mins=minutesSince(latest.startDate||latest.date,latest.timeFrom);
  return mins===null||isNaN(mins)?null:Math.max(0,mins);
}
function fmtHHMMDuration(totalMinutes){
...
function syncSleepElapsedUI(){
  ...
  var mins=babySleepElapsedMinutes(load());
  if(mins===null)return;
  el.textContent='Đã ngủ '+fmtHHMMDuration(mins);
}
