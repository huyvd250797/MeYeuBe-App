...
const DIR = '/home/claude/work/MeYeuBe-V13.3.0-Smart-Suggest';
...
{
  // Ví dụ 1: 50ml (6 giờ) · 80ml (2 ngày) · 120ml (5 ngày), bé bú 70ml → gợi ý túi 80ml
  const db = baseDb({ milkInventory: [bag('B50', 50, 6), bag('B80', 80, 48), bag('B120', 120, 120)] });
  const w = boot(db);
  const r = w.ssSuggestBags(db, 70);
  eq('chọn đúng 1 túi', r.bags.length, 1);
  eq('chọn túi 80ml (đủ lượng + hết hạn sớm nhất trong các túi đáp ứng)', r.bags[0].bagId, 'B80');
  eq('chỉ lấy đúng 70ml khỏi túi', r.bags[0].usedMl, 70);
  ok('không nhắc dùng-ngay vì túi được chọn còn 2 ngày', !/Nên dùng túi B80/.test(r.warn), 'warn=' + r.warn);
  ok('nhưng vẫn nhắc túi 50ml sắp hết hạn không được chọn', /B50/.test(r.warn), 'warn=' + r.warn);
  ok('không bỏ qua túi 50ml vì thiếu lượng, không lấy túi 120ml vì HSD xa hơn', r.bags[0].bagId === 'B80');
}
{
...
{
  // Có một túi đơn đủ lượng thì KHÔNG ghép nhiều túi (theo ví dụ 1 của tài liệu)
  const db = baseDb({ milkInventory: [bag('C50', 50, 8), bag('C40', 40, 24), bag('C100', 100, 200)] });
  const w = boot(db);
  const r = w.ssSuggestBags(db, 70);
  eq('chỉ mở 1 túi thay vì ghép 2 túi nhỏ', r.bags.map(b => b.bagId), ['C100']);
  ok('vẫn nhắc túi nhỏ sắp hết hạn dù không chọn nó', /C50/.test(r.warn), 'warn=' + r.warn);
...
console.log('\n[3] Tích hợp thật vào form Ghi nhận');
{
  ...
  eq('túi sữa được chọn sẵn theo lượng 120ml', srcs.map(s => s.bagId), ['B120']);
  eq('lấy đúng 120ml', srcs[0].usedMl, 120);
